...
YansWifiChannel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::YansWifiChannel")
    ...
    .AddAttribute("UseGSCM",
                    ...
                    BooleanValue(false),
                    MakeBooleanAccessor(&YansWifiChannel::m_useGSCM),
                    MakeBooleanChecker())
  ;
  return tid;
}
...
YansWifiChannel::GetPropagationLossModel ()
{
  return m_loss;
}
...
YansWifiChannel::Send (Ptr<YansWifiPhy> sender, Ptr<const Packet> packet, double txPowerDbm, Time duration) const
{
  ...
  int TxID = sender->GetDevice()->GetNode()->GetId();
      ...
      if (sender != (*i))
        {
          ...
          int RxID = (*i)->GetDevice()->GetNode()->GetId();
          double rxPowerDbm;
          // Due to unavailability of the NodeID in PropagationLossModel base class, we access GSCM value directly here (we can't implement it in GscmRssLossModel which inherits PropagationLossModel - Tx/Rx id are unavailable per design )
          if (m_useGSCM)
            {
              const Ptr<GscmRssLossModel>  ChModelPtr = DynamicCast<GscmRssLossModel> (m_loss);
              rxPowerDbm = ChModelPtr->GetGSCMrss(TxID, RxID);
              std::cout<< "m_useGSCM value is " << m_useGSCM << " GSCM power received: " << rxPowerDbm<< std::endl;
              std::cout<< "TxID: " << TxID << " RxID: " << RxID << std::endl;

            } else{
              rxPowerDbm = m_loss->CalcRxPower (txPowerDbm, senderMobility, receiverMobility);
              std::cout<< "m_useGSCM value is " << m_useGSCM << " power calculated: " << rxPowerDbm<< std::endl;
            }
