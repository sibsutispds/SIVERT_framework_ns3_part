...
//  std::cout<<"Spectral helper power in dBm: " << powerTx << std::endl;
  // powerTx is expressed in dBm. We must convert it into natural unit.
  double powerTxW = std::pow (10., (powerTx - 30) / 10);
//  std::cout<<"Spectral helper power in W: " << powerTxW << std::endl;

  double txPowerDensity = (powerTxW / (activeRbs.size() * 180000));
//  std::cout<<"SActive RB size: " << activeRbs.size() << std::endl;
//  std::cout<<"Spectral helper UL PSD: " << txPowerDensity << std::endl;

  for (std::vector <int>::iterator it = activeRbs.begin (); it != activeRbs.end (); it++)
    {
      int rbId = (*it);
      (*txPsd)[rbId] = txPowerDensity;
//      std::cout<<"Spectral helper RB number: " << rbId << std::endl;
//      std::cout<<"Spectral helper PSD init: " << (*txPsd)[rbId] << std::endl;
    }
