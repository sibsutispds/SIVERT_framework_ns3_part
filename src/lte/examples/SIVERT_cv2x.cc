...
#include "../flatbuffer_structures/positionSchema_single_GSCM_link_generated.h" // include for our API's flatbuffer structure
...
using namespace ns3;
using namespace SivertAPI::PosUpdNoMultiGSCM; // namespace for Position API's flatbuffers structures
...
void SetAllPositionFromAPI (NodeContainer Container, const Ptr<FixedRssLossModel> ChModelPtr, zmq::socket_t * APIconnect, double FixedUpdInterval)
{
/*
...
*/
  ...
  for (unsigned int i = 0; i < posit->size() ; i++) {
      std::cout<< "enter pos update " << std::endl;

      if (EmergencyBrake->triggered()) {

          try {
//              Ptr<Node> veh = Container.Get (i);
//              Ptr<BsmApplication> app = veh->GetApplication(0)->GetObject<BsmApplication>();
////            std::cout<< "Number of applications on the node: " <<veh->GetNApplications() << std::endl;
//              app->EEBL = true;
              std::cout<< "EBBL called. To be Implemented!" << std::endl;
            } catch (const std::exception& e) {
              std::cout<< "ERROR:" << e.what() << std::endl;
            }

        }
      // Channel model sorcing with GSCM RSS
      if (GSCMunity->UseGSCM()){
          ChModelPtr->SetRss(GSCMunity->RSS());
//          ChModelPtr->SetRss(-130.0);
          std::cout<< "Updating GSCM from Unity to NS3 with: "<< GSCMunity->RSS() << " dBm" << std::endl;
        }

      Ptr<ConstantPositionMobilityModel> mobil = Container.Get(i)->GetObject<ConstantPositionMobilityModel>();
      int veh = posit->size() - i - 1;
      mobil->SetPosition(Vector(posit->Get(veh)->x(), posit->Get(veh)->z(), posit->Get(veh)->y()));
      std::cout<< "Position update received from Unity3D for Veh: " << info->Get(i)->id()<< " Position" << " x: " <<  posit->Get(i)->x()<< " y: " << posit->Get(i)->y()<< " z: " << posit->Get(i)->z() << std::endl;
    }

  std::cout<< "Unity3D time received: " << info->Get(0)->timeStamp() << " NS3 current time: "<< Simulator::Now()<< " EEBL is set to: "<< EmergencyBrake->triggered() << std::endl;
  ...
  Simulator::Schedule (Seconds(FixedUpdInterval), &SetAllPositionFromAPI, Container, ChModelPtr, APIconnect, FixedUpdInterval);
...
main (int argc, char *argv[])
{
   ...
   * implementation in NS3.
    */
    ...
    initSocket.connect ("tcp://127.0.0.1:5555");
    int attempts = 1;
    ...
    while (attempts > 0){
            ...
            std::cout << "Received Hello Message from Unity3D with FixedUpd value: " << updStep << std::endl;
...
//        lteHelper->EnableLogComponents();
        ...
        *log_simtime->GetStream() << "Simtime;TotalRx;TotalTx;PRR" << std::endl;
        ...
        // Getting Pointer to Channel model

        const Ptr<PropagationLossModel> ChModel =lteHelper->GetDownlinkPropagationLossModel();
        const Ptr<FixedRssLossModel>  ChModelPtr = DynamicCast<FixedRssLossModel> (ChModel);
//        lteHelper->EnableLogComponents();
...
//        Simulator::Stop(MilliSeconds(simTime*1000+40));
        ...
        Simulator::Schedule (Seconds(SimStart), &SetAllPositionFromAPI, ueVeh, ChModelPtr, APIconnect_sub, UnityFixedUpd);
        Simulator::Run();
