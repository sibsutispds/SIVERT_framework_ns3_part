...
{   
    ...
    std::cout<<"BSM received with content: "<< s <<std::endl;
...
void UpdateGSCMTest ( Ptr<SivertSpectrumPropagationLossModel>  SpectrumModelPtr )
{

  std::vector<double> gscmGain;
  for (int i=0; i<30; ++i){
      gscmGain.push_back(1e-3);
    }
  SpectrumModelPtr->SetChannelFromUnity3D(0,1,gscmGain);
  SpectrumModelPtr->SetChannelFromUnity3D(0,2,gscmGain);
  SpectrumModelPtr->SetChannelFromUnity3D(1,2,gscmGain);

  Simulator::Schedule (Seconds(0.1), &UpdateGSCMTest, SpectrumModelPtr);
}
...
main (int argc, char *argv[])
{
    ...
    Config::SetDefault ("ns3::LteUeMac::SlSubchannelNum", UintegerValue (numSubchannel*numSubchannel));
    ...
    Config::SetDefault ("ns3::MultiModelSpectrumChannel::UseGSCM", BooleanValue (false));
    ...
    Config::SetDefault ("ns3::SivertModelSpectrumChannel::NumVeh", UintegerValue(3));
    Config::SetDefault ("ns3::SivertSpectrumPropagationLossModel::NumFreq", UintegerValue(30));
    ...
    *stream2->GetStream () << "Time" << std::endl;
    ...
    for(gIt=txGroups.begin(); gIt != txGroups.end(); gIt++)
        ...
        *log_tx_data->GetStream() << "TxPackets;TxTime;TxId;xPos;yPos" << std::endl;
        ...
        Ptr<SivertSpectrumPropagationLossModel>  SpectrumModelPtr = DynamicCast<SivertSpectrumPropagationLossModel> (SpectrumModelPtr_temp);
        std::vector<double> gscmGain;
        for (int i=0; i<30; ++i){
            gscmGain.push_back(1e-3);
          }
