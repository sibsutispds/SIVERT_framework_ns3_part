...
TypeId LteHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute("UseGSCM",
                  ...
                  BooleanValue(false),
                  MakeBooleanAccessor(&LteHelper::m_useGSCM),
                  MakeBooleanChecker())
  ;
  return tid;
}
...
Ptr<PropagationLossModel> LteHelper::GetDownlinkPropagationLossModel (void) const
{
  return dlPlm;
}

Ptr<PropagationLossModel> LteHelper::GetUplinkPropagationLossModel (void) const
{
  return ulPlm;
}
...
//      Ptr<PropagationLossModel> dlPlm = m_downlinkPathlossModel->GetObject<PropagationLossModel> ();
      dlPlm = m_downlinkPathlossModel->GetObject<PropagationLossModel> ();
...
//      Ptr<PropagationLossModel> ulPlm = m_uplinkPathlossModel->GetObject<PropagationLossModel> ();
      ulPlm = m_uplinkPathlossModel->GetObject<PropagationLossModel> ();
  ...
  if (propagationLoss)
    {
      ...
      if (m_useGSCM){
          const Ptr<GscmRssLossModel>  ChModelPtr = DynamicCast<GscmRssLossModel> (propagationLoss);
          int TxID = txPhy->GetDevice()->GetNode()->GetId();
          int RxID = rxPhy->GetDevice()->GetNode()->GetId();
          double propagationGainDb = ChModelPtr->GetGSCMrss(TxID, RxID);

          if (TxID!=RxID){
              std::cout<< "m_useGSCM in LTE helper 1 is " << m_useGSCM << " GSCM gain: " << propagationGainDb << std::endl;
              std::cout<< "TxID: " << TxID << " with position:" << txPhy->GetMobility()->GetPosition()<<  " RxID: " << RxID << " with position: " << rxPhy->GetMobility()->GetPosition() << std::endl;
            }
        }
      else{
          ...
          std::cout<< "m_useGSCM in LTE helper is: " << m_useGSCM << " gain: " << propagationGainDb << std::endl;
        }
  ...
  if (propagationLoss)
    {
      if (m_useGSCM){
          ...
          if (TxID!=RxID){
              std::cout<< "m_useGSCM in LTE helper 2 is " << m_useGSCM << " GSCM gain: " << propagationGainDb << std::endl;
              std::cout<< "TxID: " << TxID << " with position:" << txPhy->GetMobility()->GetPosition()<<  " RxID: " << RxID << " with position: " << rxPhy->GetMobility()->GetPosition() << std::endl;
            }
        }
      else{
