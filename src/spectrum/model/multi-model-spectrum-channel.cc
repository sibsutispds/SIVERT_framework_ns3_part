...
MultiModelSpectrumChannel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MultiModelSpectrumChannel")
    ...
    .AddTraceSource ("TxSigParams",
      ...
      BooleanValue(false),
      MakeBooleanAccessor(&MultiModelSpectrumChannel::m_useGSCM),
      MakeBooleanChecker())
  ;
  return tid;
}
...
////                          std::cout<< " Path loss added to psd: "<< pathGainLinear << " Path Loss dB: "<< pathLossDb<< " m_propagationLoss: "<< m_propagationLoss->GetTypeId() << std::endl;
...
//                          int sucarrierN = 1;
//                          std::cout<< " PSD passed from multi-model-spectrum: " << std::endl;
