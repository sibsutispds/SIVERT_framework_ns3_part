...
SivertModelSpectrumChannel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SivertModelSpectrumChannel")
    ...
    .AddAttribute("UseGSCMpathLoss",
                "Indicate if we need to expect use GSCM PathLoss RSS model from SIVERT",
                      BooleanValue(false),
                      MakeBooleanAccessor(&SivertModelSpectrumChannel::m_useGSCMPathLoss),
                      MakeBooleanChecker())
    .AddAttribute("UseGSCM",
                    "Indicate if we expect to use GSCM spectrum from SIVERT",
                    BooleanValue(true),
                    MakeBooleanAccessor(&SivertModelSpectrumChannel::m_useGSCM),
                    MakeBooleanChecker())
    .AddAttribute("NumVeh",
                    "Number of vehicles in Unity3D V2X simulation run",
                    UintegerValue (3),
                    MakeUintegerAccessor(&SivertModelSpectrumChannel::m_NumVeh),
                    MakeUintegerChecker<uint16_t> (2, 200))
  ;
  return tid;
}
...
//  std::cout<< "Setting SpectrumModel with Uid: " << rxSpectrumModelUid << std::endl;
                      ...
                      if (m_useGSCMPathLoss)
                        {
                          ...
                          if (TxID<m_NumVeh+1 && RxID<m_NumVeh+1 && TxID!=RxID){
                              std::cout<< " Sivert Propagation Loss model" << " GSCM RSS: " << propagationGainDb << std::endl;
                  ...
                  if (m_spectrumPropagationLoss)
                    {
                      ...
                      if (m_useGSCM) // We use GSCM spectrum model
                        {
                          int RxID = (*rxPhyIterator)->GetDevice()->GetNode()->GetId();
                          int TxID = txParams->txPhy->GetDevice()->GetNode()->GetId();
                          ...
                          if (TxID<m_NumVeh+1 && RxID<m_NumVeh+1) // Exclude base station with ID m_NumVeh+1 from model
                            {
//                              std::cout<< " Path Loss dB: "<< pathLossDb<< " m_propagationLoss: "<< m_propagationLoss->GetTypeId() << " Spectrum model UseGSCM: " << m_useGSCM << std::endl;
...
//                              int txPSDsize = txParams->psd->ValuesBegin() - txParams->psd->ValuesEnd();
                          ...
                          else // Base station instead will use implementaion of virtual CalcRxPowerSpectralDensity, where we use Friis model
                            {
                              double d = txMobility->GetDistanceFrom (receiverMobility);
...
//                          std::cout<< " PSD after SIVERT-model-spectrum-chanel: "<< " txPSDsize " << txPSDsizeA << " rxPSDsize "<< rxPSDsizeA << " m_spectrumPropagationLoss: " << m_spectrumPropagationLoss << std::endl;
                        }
                      else
                        {
                          int RxID = (*rxPhyIterator)->GetDevice()->GetNode()->GetId();
...
//                          int rxPSDsize = rxParams->psd->ValuesBegin() - rxParams->psd->ValuesEnd();
//                          int txPSDsize = txParams->psd->ValuesBegin() - txParams->psd->ValuesEnd();
//                          std::cout<< "PSD size tx: " << txPSDsize << " PSD size rx: " << rxPSDsize << std::endl;
              ...
              if (netDev)
                {
                  ...
                  Simulator::ScheduleWithContext (dstNode, delay, &SivertModelSpectrumChannel::StartRx, this,
                                                  rxParams, *rxPhyIterator);
                }
              else
                {
                  // the receiver is not attached to a NetDevice, so we cannot assume that it is attached to a node
                  Simulator::Schedule (delay, &SivertModelSpectrumChannel::StartRx, this,
                                       rxParams, *rxPhyIterator);
                }
            }
        }
