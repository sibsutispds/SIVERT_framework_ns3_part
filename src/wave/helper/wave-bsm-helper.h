...
public:
  ...
  void InstallAPI (Ipv4InterfaceContainer & i,
                Time totalTime,          // seconds
                uint32_t wavePacketSize, // bytes
                Time waveInterval,       // seconds
                double gpsAccuracyNs,    // clock drift range in number of ns
                ...
                int chAccessMode,        // channel access mode (0=continuous; 1=switching)
                Time txMaxDelay,        // max delay prior to transmit
                Time StartBSS);         // start time of BSS service received from Unity3D

  /**
   * \brief Returns the WaveBsmStats instance for SIVERT API
   * \return the WaveBsmStats instance for SIVERT framework
   */
