...
public:
  ...
  Ptr<Socket> recvSink;
  Ptr<Socket> GetSocketPointer();
  void CheckCITSEvent(bool brake);
  void SetEEBL(bool val);
  void SetStartTimeFromUnity(Time Stime);
  bool EEBL = false;
  int EEBLtriggerVehID;
  /**
...
private:
  ...
  Time UnitysStartTime;

  TracedCallback<Ptr<const Packet>, Ptr<Node>> m_BSMsentTrace; //!< Trace for received packet event
  TracedCallback<Ptr<const Packet>, Ptr<Node>, Ptr<Node>> m_BSMreceivedTrace; //!< Trace for received packet event
