...
BsmApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BsmApplication")
    ...
    .AddTraceSource ("BSMreceived",
                     "Vehicle node received a BSM",
                     MakeTraceSourceAccessor (&BsmApplication::m_BSMreceivedTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("BSMsent",
                     "Vehicle node sent a BSM",
                     MakeTraceSourceAccessor (&BsmApplication::m_BSMsentTrace),
                     "ns3::Packet::TracedCallback")
    ;
  return tid;
}
...
//  Time startTime = Seconds (1.0);
  Time startTime = UnitysStartTime;
  ...
  recvSink = Socket::CreateSocket (GetNode (m_nodeId), tid);
  ...
  const int range_to    = d_ns;
//  std::random_device                  rand_dev;
//  std::mt19937                        generator(rand_dev(curTime));
//  std::uniform_int_distribution<int>  distr(range_from, range_to);
//  Time txDelay = NanoSeconds (distr(generator));

  std::uniform_int_distribution<int> rng_range( 0, d_ns );
  std::random_device rd;
  std::mt19937 rng_mt( curTime +  recvSink->GetNode ()->GetId ());

  Time txDelay = NanoSeconds (rng_range( rng_mt ));

//  Time txDelay = NanoSeconds (m_unirv->GetInteger (0, d_ns));
  m_prevTxDelay = txDelay + NanoSeconds (rng_range( rng_mt ));

  Time txTime = startTime + tDrift + txDelay;
  // schedule transmission of first packet
  std::cout<< "BSM application is on node: " << recvSink->GetNode ()->GetId () << "start is schedulled at: "<< txTime << std::endl;
  ...
  Simulator::ScheduleWithContext (recvSink->GetNode ()->GetId (),
                                  txTime, &BsmApplication::CheckCITSEvent, this, EEBL);
}
  ...
  if (pktCount > 0)
    {
      ...
      std::cout<< "Sending BSM from node " << sendingNodeId << " at: " << Simulator::Now()<< std::endl;
      ...
      /*int senderMoving = m_nodesMoving->at (txNodeId);*/ // !!!!!! what is that??
      int senderMoving = 1;
      if (senderMoving != 0)
        {
          // send it!
          std::stringstream aa;
          if ( (txNodeId == 0) && (Simulator::Now().GetSeconds() < 0.0)){
              aa << "Brake";
            }
          else {
              aa<< "BeaconID: "<< m_waveBsmStats->GetTxPktCount()<<" VehID: "<<txNodeId<< " Position x: "<< txPosition->GetPosition().x << " y: "<< txPosition->GetPosition().y << " z: "<< txPosition->GetPosition().z << " Time: "<< Simulator::Now().GetNanoSeconds();

            }
//          aa("VehID: ",txNodeId, "Position: ", txPosition->GetPosition(), "Time: ", Simulator::Now());
//          aa<< "BeaconID: "<< m_waveBsmStats->GetTxPktCount()<<" VehID: "<<txNodeId<< " Position x: "<< txPosition->GetPosition().x << " y: "<< txPosition->GetPosition().y << " z: "<< txPosition->GetPosition().z << " Time: "<< Simulator::Now()<< std::endl;
          std::string msg = aa.str();
//          uint8_t *buffer = msg;
          std::cout<< "Sent BSM with: " << "BeaconID: "<< m_waveBsmStats->GetTxPktCount()<<" VehID: "<<txNodeId<< " Position x: "<< txPosition->GetPosition().x << " y: "<< txPosition->GetPosition().y << " z: "<< txPosition->GetPosition().z << " Time: "<< Simulator::Now().GetNanoSeconds();


//          socket->Send (Create<Packet> (reinterpret_cast<const uint8_t*>(msg.c_str())  , pktSize));
          socket->Send (Create<Packet> (reinterpret_cast<const uint8_t*>(msg.c_str())  , msg.size()));
          m_BSMsentTrace(Create<Packet> (reinterpret_cast<const uint8_t*>(msg.c_str())  , msg.size()), txNode);
          // count it
...
void BsmApplication::ReceiveWavePacket (Ptr<Socket> socket)
{
  ...
  Ptr<Packet> packet;
  Address senderAddr;
//  std::cout << "Trying to receive beacon at app level. ID: " << m_waveBsmStats->GetTxPktCount() << std::endl;
              ...
              if (addr.GetIpv4 () == m_adhocTxInterfaces->GetAddress (i) )
                {
                  ...
                  m_BSMreceivedTrace (packet, rxNode, txNode);
                                              ...
                                              Ptr<Node> rxNode)
{
  ...
  // it is not a candidate to receive a packet
//  int rxNodeId = rxNode->GetId ();
//  std::cout<< "Received packet from: " << rxNodeId << std::endl;
//  int receiverMoving = m_nodesMoving->at (rxNodeId);
  int receiverMoving = 1;
  if (receiverMoving == 1)
    {
...
Ptr<Socket>
BsmApplication::GetSocketPointer()
{
  return this->recvSink;
}

void
BsmApplication::CheckCITSEvent(bool brake)
{
  if(EEBL)
    {
      std::cout << "Harsh braking. Generate the EEBL message!" << std::endl;
      std::stringstream aa;
      aa << "Brake" << EEBLtriggerVehID;
      std::string msg = aa.str();
      recvSink->Send (Create<Packet> (reinterpret_cast<const uint8_t*>(msg.c_str())  , msg.size()));

    }
  Simulator::ScheduleWithContext (recvSink->GetNode ()->GetId (),
                                  Seconds(0.1), &BsmApplication::CheckCITSEvent, this, EEBL);
}

void
BsmApplication::SetEEBL(bool val)
{
  EEBL = val;
}

void BsmApplication::SetStartTimeFromUnity(Time Stime)
{
  UnitysStartTime = Stime;
}
