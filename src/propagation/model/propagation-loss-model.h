 ...
 * \ingroup propagation
 *
 * \brief Sets and returns the GSCM model computed value from Unity
 *  in SIVERT simulation framework
 *
 * The received power is constant independent of the transmit power.  The user
 * must set received power level through the Rss attribute or public
 * SetRss() method.  Note that if this loss model is chained to other loss
 ...
 * that precede it in the chain.
 */
    class GscmRssLossModel : public PropagationLossModel
    {
    public:
        /**
         * \brief Get the type ID.
        ...
        GscmRssLossModel ();
        virtual ~GscmRssLossModel ();
        /**
         * \param rss (dBm) the received signal strength
         *
         * Set the received signal strength (RSS) in dBm. GSCM RSS is received from Unity SIVERT framework.
         */
        void SetRss (int TxID, int RxID, double rss);
        double GetGSCMrss(int TxID, int RxID);
    ...
    private:
         ...
         * Defined and unimplemented to avoid misuse
         */
        GscmRssLossModel (const GscmRssLossModel &);
        /**
         * \brief Copy constructor
         *
         * Defined and unimplemented to avoid misuse
         * \returns
         */
        GscmRssLossModel & operator = (const GscmRssLossModel &);
        ...
        virtual int64_t DoAssignStreams (int64_t stream);
        double m_rss; //!< the received signal strength
        double m_gscm_rss_lookup[100][100] = {}; // lookup table containing links computations received from GSCM Unity SIVERT.
    };
