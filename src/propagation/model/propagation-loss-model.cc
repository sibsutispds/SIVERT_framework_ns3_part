...
// ------------------------------------------------------------------------- //
    ...
    NS_OBJECT_ENSURE_REGISTERED (GscmRssLossModel);

    TypeId
    GscmRssLossModel::GetTypeId (void)
    {
        static TypeId tid = TypeId ("ns3::GscmRssLossModel")
                .SetParent<PropagationLossModel> ()
                .SetGroupName ("Propagation")
                .AddConstructor<GscmRssLossModel> ()
                .AddAttribute ("Rss", "The fixed receiver Rss.",
                               DoubleValue (-150.0),
                               MakeDoubleAccessor (&GscmRssLossModel::m_rss),
                               MakeDoubleChecker<double> ())
        ;
        return tid;
    }
    GscmRssLossModel::GscmRssLossModel ()
            : PropagationLossModel ()
    {
        for (int i = 0; i < 100 ; ++i) {
            for (int j = 0; j < 100; ++j) {
                m_gscm_rss_lookup[i][j] = -400;
            }
        }
    }
    ...
    void
    GscmRssLossModel::SetRss (int TxID, int RxID, double rss)
    {
//        m_rss = rss;
        m_gscm_rss_lookup[TxID][RxID] = rss;
    }

    double
    GscmRssLossModel::DoCalcRxPower (double txPowerDbm,
                                      Ptr<MobilityModel> a,
                                      Ptr<MobilityModel> b) const
    {
        return m_rss;
    }
    double GscmRssLossModel::GetGSCMrss(int TxID, int RxID)
    {
        return m_gscm_rss_lookup[TxID][RxID];
    }

    int64_t
    GscmRssLossModel::DoAssignStreams (int64_t stream)
    {
        return 0;
    }
